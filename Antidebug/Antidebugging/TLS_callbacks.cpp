...
VOID WINAPI tls_callback(PVOID hModule, DWORD dwReason, PVOID pContext)
{
	...
	{
		OutputDebugString("TLS callback: thread attach");
	...
	{
		OutputDebugString("TLS callback: process attach");
...
{
	OutputDebugString("TLS callback: dummy thread launched");
	return 0;
}
...
BOOL TLSCallbackThread()
{
	...
	{
		OutputDebugString("TLS callback: couldn't start dummy thread");
	}
	...
	if (fuse >= BLOWN)
	{
		OutputDebugString("TLSCallbackThread timeout on event creation.");
		return TRUE;
	}
	...
	if (waitStatus != WAIT_OBJECT_0)
	{
		if (waitStatus == WAIT_FAILED)
			OutputDebugString("TLSCallbackThread wait failed.");
		else if (waitStatus == WAIT_ABANDONED)
			OutputDebugString("TLSCallbackThread wait abandoned.");
		else
			OutputDebugString("TLSCallbackThread timeout on event wait.");
		return TRUE;
	}

	if (tls_callback_thread_data != 0xDEADBEEF)
		OutputDebugString("TLSCallbackThread data did not match.");
	else
		OutputDebugString("All seems fine for TLSCallbackThread.");
...
BOOL TLSCallbackProcess()
{
	...
	if (fuse >= BLOWN)
	{
		OutputDebugString("TLSCallbackProcess timeout on event creation.");
		return TRUE;
	}
	...
	{
		if (waitStatus == WAIT_FAILED)
			OutputDebugString("TLSCallbackProcess wait failed.");
		else if (waitStatus == WAIT_ABANDONED)
			OutputDebugString("TLSCallbackProcess wait abandoned.");
		else
			OutputDebugString("TLSCallbackProcess timeout on event wait.");
		return TRUE;
	}

	if (tls_callback_process_data != 0xDEADBEEF)
		OutputDebugString("TLSCallbackProcess data did not match.");
	else
		OutputDebugString("All seems fine for TLSCallbackProcess.");
